# orchestrator/git_expert.py (GitAgent worker)
...
from orchestrator.agents.git_agent import git_agent
...
QUEUE = "queue:GitAgent"
LOG   = "logs:GitAgent"

# ─── OpenAI client (uses SWIFT_AGENT_API_KEY) ────────────────────────
client = oa("GIT_AGENT_API_KEY")  # returns an `OpenAI` client
...
# ─── main loop ─────────────────────────────────────────────────────────
async def main() -> None:
    print("GitAgent worker booted", flush=True)
    ...
    while True:
        ...
        try:
            ...
            resp = client.chat.completions.create(
                model=git_agent.model,
                messages=[
                    {"role": "system", "content": git_agent.instructions},
                    {"role": "user", "content": utext},
                ],
                tools=git_agent.functions,
                tool_choice="auto",
            )
            ...
            result = await Runner.run(git_agent, input={"name": name, "args": args})
            ...
            if "path" in result and "content" in result and "note" in result:
                ...
                await redis.lpush("suggestions:GitAgent", json.dumps(suggestion))
