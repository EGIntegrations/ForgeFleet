...
"""
Swift_Agent – queues every code/doc change for human approval.
...
# ───────────────────────────────────────────────────────────────────────
VECTOR_ID = os.getenv("SWIFT_VECTOR_ID")
if not VECTOR_ID:
    raise RuntimeError("Set SWIFT_VECTOR_ID in your .env")

REDIS_URL = os.getenv("REDIS_URL", "redis://redis:6379/0")
redis_sync = Redis.from_url(REDIS_URL, decode_responses=True)   # sync handle
SUG_KEY    = "suggestions:SwiftAgent"
...
swift_agent = AssistantAgent(
    name         = "SwiftAgent",
    model        = "gpt-4.1-nano-2025-04-14",
    instructions = (
        "You are a ForgeFleet support assistant, specialzing in the swift coding language.\n"
